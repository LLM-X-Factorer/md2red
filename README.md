...
- **交互式预览** — 浏览器中拖拽排序、删除卡片、编辑标题/摘要/标签，一键复制文案到剪贴板
- **导出打包** — 一键下载 zip（所有图片 + 发布文案.txt），标签自动转为小红书 `#标签#` 格式
...
- **编辑摘要** — 修改笔记正文
- **管理标签** — 添加或删除标签
- **一键复制** — 点击「一键复制文案」将标题+正文+标签复制到剪贴板，直接粘贴到小红书

### 第四步：导出发布

在预览页面点击 **「导出图片+文案」** 下载 zip 包，包含所有图片和发布文案（标签自动转为 `#标签#` 格式），然后手动上传到小红书或其他平台。
...
content:
  ...
  minCards: 5                # 最少卡片数（LLM 生成不足时自动补充）
...
- **预览编辑** — 卡片轮播、编辑标题/摘要/标签、导出 zip
- **生成历史** — 记录表格，每条可直接导出或查看
- **设置** — LLM 提供商、主题、品牌色、内容风格等配置
